...
let bind_rhs_with_value_kind str (var, kind) exp body =
  match body with
  | Unguarded body -> Unguarded (bind_with_value_kind str (var, kind) exp body)
  | Guarded { patch_guarded; free_variables = free } ->
      match exp with
      | Lvar var' when Ident.same var var' -> body
      | _ ->
          let patch_guarded ~patch =
            Llet (str, kind, var, exp, patch_guarded ~patch) in
          let free_variables =
...
let bind_alias p id ~arg ~action =
  ...
  bind_rhs_with_value_kind Alias (id, k) arg action
  ...
  let rec do_cases = function
    ...
    | ((p, patl), action) :: rem -> (
        ...
        | `Or _ ->
            ...
            in
            let patbound_idents = Typedtree.pat_bound_idents_full orp in
            ...
            *)
            ...
            let patbound_idents =
              ...
              | Known pm_fv ->
                  List.filter
                    (fun (id, _, _) -> Ident.Set.mem id pm_fv)
                    patbound_idents
            in
            let patbound_action_vars =
              List.map
                (fun (id, _, ty) -> (id, Typeopt.value_kind orp.pat_env ty))
                patbound_idents
            in
...
let simple_for_let ~scopes loc param pat body =
  compile_matching ~scopes loc ~failer:Raise_match_failure
    None param [ (pat, Unguarded body) ] Partial
